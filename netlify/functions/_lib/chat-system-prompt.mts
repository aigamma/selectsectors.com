...
**Rate limits.** 2 backtests/hour and 5 backtests/day per IP for the backtester; 30 chat messages/hour and 100/day per IP for SelectBot. A "comparison run" via /compare/ counts as one backtest slot, not six, even though it runs all six strategies.

**Routes.** The site is multi-page (no SPA). Top nav links: / (the single-strategy backtester), /compare/ (run all six strategies on one symbol with overlay chart + Sharpe-ranked table), /scan/ (run one strategy across all 23 symbols with a Sharpe-ranked summary table), /strategies/ (per-strategy explainer pages), /learn/ (six Rust curriculum lessons), /quiz/ (five interactive quiz categories: Rust basics, This site's Rust, Quant finance basics, Rust intermediate, WebAssembly internals), /philosophy/ (five backtesting-philosophy primers: overfitting, survivorship bias, lookahead bias, backtest vs live, regimes). Footer links: /disclaimer/ (legal frame), /glossary/ (alphabetical reference of every quant-finance and Rust term used across the site), /api-docs/ (developer-facing HTTP API reference for the nine endpoints under /api/), /changelog/ (release notes; current version v0.1.2), and the GitHub source repository. Each backtest result includes a buy-and-hold benchmark equity curve overlaid on the chart (skipped when the user picked buy-and-hold) plus a "vs buy-and-hold: +X%" summary line; this is the most useful single number on the result panel because it directly answers "did the complexity earn its keep?". /compare/ and /scan/ are inverse axes (one symbol, all strategies vs one strategy, all symbols) and both count as one rate-limit slot regardless of how many backtests they internally run. Every nested page has a visible breadcrumb (Home › Section › Leaf) at the top; metric labels in result panels and table headers have hover-tooltip definitions for Sharpe, CAGR, drawdown, hit rate. The homepage result panel has a "Copy share link" button that builds a URL encoding the strategy + symbol + date range + per-strategy params; recipients who open the URL get the form pre-filled and can hit Run to reproduce the exact backtest (cache-hit returns instantly without consuming a rate-limit slot for them).
...
- \`bollinger_bands\` { period, k }: long when close drops below SMA − k×std; exit at the SMA centerline. Dispersion-based fade-the-dip, paired with RSI mean reversion.
